...
main_raw2 <- function(x,
        ...
        #                                 penalty.factor=penalty.factor)
        # lambda.vec.enet <- lapply( lambda.list.enet, function(x) seq(median(x), max(x), length.out=10) )
        # seq.lambda = list(enet=lambda.vec.enet, mnet=NULL)
        # seq.df = floor(seq(nrow(y)/2, nrow(y), length.out=10))
        # K = 100
        # psub <- 0.5
        ...
        if( is.null(seq.df)) seq.df <- floor(seq(floor(nrow(y)/2), floor(nrow(y)), length.out=10))
        ...
        if( include.unet_df ){
                ...
                beta.array.unet_sum <- beta.array.unet_df2 <- beta.array.unet_df
        }
        ...
        if( include.unet_df ){
                ...
                res.lamb <- NULL
                for( lamb in 1:dim(beta.array.enet)[3] ){
                        res.lamb <- cbind(res.lamb,
                                          png.recalib.varsel( beta.array.enet[,1,lamb,] ) )
                }

                for( k in 1:ncol(y) ){
                        beta.array.unet_df2[,1,,k] <- apply( res.lamb, 2, function(x) x/max(x) )
                }
        ...
        out <- list(NULL)
        if( include.enet ) out <- append(out, list(enet=( beta.array.enet) ))
        if( include.mnet ) out <- append(out, list(mnet=( beta.array.mnet) ))
        if( include.unet ) out <- append(out, list(unet=( beta.array.unet) ))
        if( include.unet_df ){
                out <- append(out, list(unet_sumdf=( beta.array.unet_sum) ))
                out <- append(out, list(unet_df=( beta.array.unet_df) ))
                out <- append(out, list(unet_df2=( beta.array.unet_df2) ))
        }

        # cbind(
        #         apply( beta.array.unet_df, 1, max ) %>% order(decreasing=TRUE) ,
        #         apply( beta.array.unet_df2, 1, max ) %>% order(decreasing=TRUE)
        # ) %>% head(30)
        #
        # TOP=100
        # png.venn2(
        #         apply( beta.array.unet_df, 1, max ) %>% order(decreasing=TRUE) %>% head(TOP),
        #         apply( beta.array.unet_df2, 1, max ) %>% order(decreasing=TRUE) %>% head(TOP)
        #         )
