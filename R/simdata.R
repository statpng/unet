simdata <- function(N, P, Q, gamma=NULL,
                   Y.type=c("continuous", "binary", "multiclass", "mixed"), n.multiclass = NULL,
                   snp.rho, maf.min, y.rho, scenario = c("scenarioA", "scenarioB", "scenarioC")){
  # N=100; P=10000; Q=8; gamma = 0.5;
  ...
  if( gamma < 1 & gamma > 0 ) gamma <- gamma * Q

  if( Y.type=="multiclass" & is.null(n.multiclass) ){
    stop( "Need n.multiclass for multiclass-type data")
  }

  if( is.null(gamma) ) gamma <- Q
  ...
  cat("Phenotypic correlation value =", y.rho, "\n")
  ...
  if ( scenario == "scenarioA" ){ ## Setting46-4 : scenario2

    beta <- replicate( Q, rep(0, P) )
    for( mm in wh.true ){
      if( mm %% 2 == 0 ){
        beta[ 100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 400+seq_len(length(smallbeta)), mm] <- smallbeta
      } else {
        beta[ 100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 300+seq_len(length(smallbeta)), mm] <- smallbeta
  ...
  } else if ( scenario == "scenarioB" ){ ## Setting46-2 : scenario1
    ...
    for( mm in wh.true ){
      if( mm %% 2 == 0 ){
        ...
        beta[ 100+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 200+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 300+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 400+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
      }
    }

  } else if ( scenario == "scenarioC" ){ ## Setting46-5_BetaUp

    beta <- replicate( Q, rep(0, P) )
    for( mm in wh.true ){
      if( mm %% 2 == 0 ){
        beta[ 1100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 1200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 1300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 1400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 2100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 2200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 2300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 2400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 3100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 3200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 3300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 3400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 4100+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 4200+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 4300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 4400+seq_len(length(smallbeta)), mm] <- smallbeta
      } else {
        beta[ 1100+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 1200+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 1300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 1400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 2100+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 2200+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 2300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 2400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 3100+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 3200+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 3300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 3400+seq_len(length(smallbeta)), mm] <- smallbeta

        beta[ 4100+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 4200+seq_len(length(smallbeta)), mm] <- rev(smallbeta)
        beta[ 4300+seq_len(length(smallbeta)), mm] <- smallbeta
        beta[ 4400+seq_len(length(smallbeta)), mm] <- smallbeta
      }
    }
  ...
  cat("The indices of outcome-related variants :", unique( which( beta != 0, arr.ind=TRUE )[,1] ), "\n")
  cat("The indices of phenotypes corresponding to the outcome-related variants :", unique( which( beta != 0, arr.ind=TRUE )[,2] ), "\n")
  cat("Effect sizes : (", unique( beta[which( beta != 0, arr.ind=TRUE )] ), ") x 4 \n")

  cat("A structure of the effect sizes : \n")
  print( matrix( beta[ which( beta != 0, arr.ind=TRUE ), drop=F ], ncol=gamma ) )
  ...
  } else if( Y.type == "multiclass" ){
    for( colcol in 1:ncol(Z) ){
      for( catcat in 1:n.multiclass ){
        qntl <- quantile(Z[,colcol], ( 0:n.multiclass / n.multiclass ))
        Y[Z[,colcol] >= qntl[catcat], colcol] <- catcat
        # IL <- exp(Z[,colcol]) / (1 + exp(Z[,colcol]))
        # Y[,colcol] <- as.factor( rbinom(length(IL), 1, IL) )
      }
    }
  ...
  if( Y.type == "mixed" ){
    Family <- rep(c("binomial", "gaussian"), each=ncol(Y)/2)
  } else if( Y.type == "binary" ){
    Family <- rep("binomial", ncol(Y))
  } else if( Y.type == "continuous" ){
    Family <- rep("gaussian", ncol(Y))
  } else if( Y.type == "multiclass" ){
    Family <- rep("multinomial", ncol(Y))
  }

  Data <- list(snp=snp, y=Y, maf=SNP$MAF, beta=beta, true=unique( which( beta != 0, arr.ind=TRUE )[,1] ), family=Family )
